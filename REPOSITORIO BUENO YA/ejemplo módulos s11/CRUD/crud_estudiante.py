...
def validar_contra( usuario, usuarios):
    
    for i in range(3):
        contra=pwinput.pwinput(prompt='Ingrese su contraseña: ', mask='*')
...
def inicio():
    ...
    acceso=validar_contra(usuario, usuarios)
    if acceso:
        return menu()
    else:
        exit()
    #intento = getpass.getpass('Ingrese su contraseña: ')
